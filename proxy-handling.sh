...
run_without_proxy_then_with_fallback() {
    ...
    local safe_cmd
    safe_cmd="$(sanitize_cmd "$@")"

    echo "[INFO] Attempting without proxy: $safe_cmd"
    ...
    while [ "$retries" -lt "$max_retries" ]; do
        ...
        echo "[WARN] Retry $((retries + 1)) failed..."
        ((retries++))
    done

    echo "[INFO] Retrying with proxy: $safe_cmd"

    # restore only non-empty proxies
    local env_vars=()
    [ -n "$_HTTP_PROXY" ] && env_vars+=(HTTP_PROXY="$_HTTP_PROXY")
    [ -n "$_HTTPS_PROXY" ] && env_vars+=(HTTPS_PROXY="$_HTTPS_PROXY")
    [ -n "$_http_proxy" ] && env_vars+=(http_proxy="$_http_proxy")
    [ -n "$_https_proxy" ] && env_vars+=(https_proxy="$_https_proxy")
